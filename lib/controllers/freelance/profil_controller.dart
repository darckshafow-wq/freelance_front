...
class ProfilController {
  ...
  static int? resolveUserId(String? rawUserId, {int? currentUserId}) {
    dev.log('[ProfilController] resolveUserId called with rawUserId: $rawUserId, currentUserId: $currentUserId');
  ...
  Future<UserModel?> getUserInfo(int userId) async {
    ...
    if (userId <= 0) {
      dev.log('[ProfilController] getUserInfo - userId invalide, fallback sur /users/me');
    ...
    final response = await _apiClient.get<UserModel>(
      ...
      parser: (json) {
        ...
        dev.log('[ProfilController] getUserInfo - Parsed UserModel: ID=${model.id}, Name="${model.fullName}", Email="${model.email}"');
        return model;
      },
    );

    if (response.isSuccess && response.data != null) {
      dev.log('[ProfilController] getUserInfo SUCCESS: ID=${response.data!.id}');
      return response.data;
    }

    dev.log('[ProfilController] getUserInfo FAILED for userId=$userId, trying /users/me fallback');
    return await _fetchCurrentUserViaMe();
  }
  ...
  Future<UserModel?> _fetchCurrentUserViaMe() async {
    ...
    if (meResponse.isSuccess && meResponse.data != null) {
      dev.log('[ProfilController] _fetchCurrentUserViaMe SUCCESS: ID=${meResponse.data!.id}');
      return meResponse.data;
    }
    dev.log('[ProfilController] _fetchCurrentUserViaMe FAILED: ${meResponse.message}');
    return null;
  }
  ...
  Future<FreelanceStatsModel> loadFullProfile({required int userId}) async {
    ...
    if (effectiveUserId <= 0) {
      dev.log('[ProfilController] loadFullProfile - userId invalide, résolution via /users/me');
      final me = await _fetchCurrentUserViaMe();
      if (me == null || me.id <= 0) {
        dev.log('[ProfilController] loadFullProfile - Impossible de résoudre userId, returning empty profile.');
        return FreelanceStatsModel.empty();
      }
      effectiveUserId = me.id;
      dev.log('[ProfilController] loadFullProfile - userId résolu depuis /users/me: $effectiveUserId');
    }

    // ── Lance les 3 requêtes en parallèle ────────────────────────────────────
    dev.log('[ProfilController] loadFullProfile - Dispatching parallel requests for stats, applications, and reviews...');
    ...
    if (applicationsResponse.isSuccess && applicationsResponse.data != null) {
      ...
      dev.log('[ProfilController] candidatures OK: count=${applications.length}');
      for (var app in applications) {
        dev.log('[ProfilController] - Candidature: ID=${app.id}, TaskID=${app.taskId}, Budget=${app.proposedBudget}, Status=${app.status}');
      }
    } else {
      dev.log(
    ...
    if (reviewsResponse.isSuccess && reviewsResponse.data != null) {
      ...
      for (var rev in reviews) {
        dev.log('[ProfilController] - Avis: ID=${rev.id}, TaskID=${rev.taskId}, Rating=${rev.rating}, Comment="${rev.comment}"');
      }
    } else {
    ...
    final aggregateStats = FreelanceStatsModel.fromStatsJson(
      statsJson,
      applications: applications,
      reviews: reviews,
    );
    dev.log('[ProfilController] loadFullProfile SUCCESS - Merged FreelanceStatsModel: completed=${aggregateStats.tasksDone}, inProgress=${aggregateStats.tasksInProgress}, rating=${aggregateStats.averageRating}');
